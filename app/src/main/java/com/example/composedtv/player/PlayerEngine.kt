...
class PlayerEngine(private val context: Context) {
    ...
    fun applyPlaybackSettings(s: com.example.composedtv.viewmodel.PlaybackSettings) {
        ...
        proxyReviveMax = s.reviveMax
        Log.d(TAG, "applyPlaybackSettings: stuck=${stuckTimeoutMs} hedge=${raceHedgeMs} revive=${proxyReviveMax}")
    }
    ...
    private fun playUrl(url: String, country: String = "") {
        if (url.isBlank()) {
            handlePlayFailure()
            return
        }
        currentPlayingUrl = url
        val attempts = planPlay(url, country)
        Log.d(TAG, "planPlay($url) -> $attempts country=$country")
    ...
    private fun reviveProxyCandidate(currentWinnerExo: ExoPlayer) {
        ...
        Log.d(TAG, "reviveProxyCandidate: 直连继续播 + 代理并行竞速")
    }
